...
| Ingestion     | TypeScript (same repo, `scripts/` + `src/services/ingest/`) — see [ADR 0003](./0003-ingestion-typescript.md) | Keeps the toolchain uniform; `youtubei.js`, `@mozilla/readability`, and the Reddit public JSON API cover our sources. Python `yt-dlp` / `youtube-transcript-api` remain an optional transcript fallback only. |
