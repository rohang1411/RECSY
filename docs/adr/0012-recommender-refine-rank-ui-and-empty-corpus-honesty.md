...
> **Superseded in part by [ADR 0014](0014-automated-ingestion-curation.md).**
> The empty-corpus short-circuit still stands, but the user-facing message is no
> longer the developer-oriented "run `pnpm ingest --phone <slug>`" copy. It is
> generated by `buildNoContextMessage(phoneMeta)` in
> `src/services/chat/answer.ts` and is time-aware: it names brand + model,
> mentions days-since-last-ingest, and surfaces the next scheduled refresh
> window. Automated tiered ingestion means users should rarely see this
> message at all.
