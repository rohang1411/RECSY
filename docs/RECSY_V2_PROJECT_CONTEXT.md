...
| Automated catalog refresh foundation       | ▲      | 8     | Catalog schema/migration, legacy identity backfill, no-LLM Wikidata candidate staging, report CLI; safe auto-promotion waits for a structured spec source ([plan](./ImplementationPlans/automated-phone-catalog-refresh-pipeline.md)) |
...
- **Pure catalog modules** - identity normalization, snapshot hashing,
  ...
  Wikidata adapter are covered by focused Vitest suites under
...
but intentionally does not promote Wikidata-only rows into `phones`.

| Item                           | Acceptance criteria                                                                                                                                             | Difficulty |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| Non-LLM structured spec source | Add one trusted source path (OEM extractor with fixtures or licensed API) that fills every required `PhoneSpecSchema` field for recent phones without LLM calls | Hard       |
| Transactional promotion        | Promote validated candidates into `phones`, identities, configurations, claims, aliases, and media tables idempotently; duplicate/config variants are blocked   | Hard       |
| Hot-launch ingest bootstrap    | Newly promoted active phones either set `next_ingest_at = NULL` or dispatch `ingest-on-new-phone.yml` behind an env flag                                        | Medium     |
| Catalog workflow + resume      | Scheduled/manual `catalog:refresh` supports resume, request budgets, new-promotion caps, and reports quarantine counts                                          | Medium     |
