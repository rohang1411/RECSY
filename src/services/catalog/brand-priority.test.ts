...
describe('catalog brand priority', () => {
  it('ranks mainstream smartphone companies and sub-brands first', () => {
    ...
    expect(brandPriorityRank('Nothing')).toBe(3);
    expect(brandPriorityRank('CMF by Nothing')).toBe(3);
    expect(brandPriorityRank('OnePlus')).toBe(4);
    expect(brandPriorityRank('iQOO')).toBe(5);
    expect(brandPriorityRank('Redmi')).toBe(6);
    expect(brandPriorityRank('Infinix')).toBe(9);
  });
